// Package manifest embeds pi-zero-2w's pinned third-party artifact manifest
// (manifest.json, in this same directory) so the board profile in
// internal/boards/pizero2w can consume it without a runtime file read. See
// bean gosd-eu2x for the manifest's content and pinning rationale: GPU boot
// firmware from raspberrypi/firmware, and WiFi firmware (plus the board
// alias names it must be duplicated under) from RPi-Distro/firmware-nonfree.
package manifest
...
// WifiFirmware is the Zero 2 W's WiFi firmware group: the real blob sets

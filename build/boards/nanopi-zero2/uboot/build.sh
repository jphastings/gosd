#!/usr/bin/env bash
# Builds mainline U-Boot (idbloader.img + u-boot.itb) for the FriendlyElec
# NanoPi Zero2, inside Docker, and writes the artifacts to ./out/.
#
# Usage: ./build.sh
#
# Pinned inputs (edit here / in manifest.json to bump versions):
#   - U-Boot release tag: UBOOT_TAG below. Currently a v2026.07-rc tag --
#     see the "Pinning an RC" note in this board's README before bumping.
...
IMAGE_TAG="gosd-nanopi-zero2-uboot-build"
...
echo "Building U-Boot ${UBOOT_TAG} for nanopi-zero2 (rkbin @ ${RKBIN_COMMIT:0:12})..."

...
// to reach the engine, so these pin that it gets the same substitution and
// pristine-verification behaviour once it's there.
describe("the reserved config file", () => {
  function fixtureWithConfigRegion(): { image: Uint8Array; manifest: Manifest } {
    ...
    return {
      ...
      manifest: {
        ...
        config: {
          path: "gosd.toml",
          size: 128,
          sha256: sha256Hex(pristine),
          ranges,
          pristine: new TextDecoder().decode(pristine),
        },
      },
    };
  }
  ...
  it("substitutes its bytes and leaves the rest of the image alone", async () => {
    const { image, manifest } = fixtureWithConfigRegion();
    const replacement = new Uint8Array(128).fill(0x41);

    const out = await collect(
      patchStream(readableFrom([image]), manifest, new Map([["gosd.toml", replacement]])),
    );
  ...
  it("verifies it is pristine first, naming it in a way a caller can act on", async () => {
    ...
    const tampered: Manifest = {
      ...manifest,
      config: { ...manifest.config!, sha256: "b".repeat(64) },
    };

    await expect(
      collect(
        patchStream(readableFrom([image]), tampered, new Map([["gosd.toml", new Uint8Array(128)]])),
      ),
    ).rejects.toThrow(/reserved gosd.toml is not pristine/);
  });
});

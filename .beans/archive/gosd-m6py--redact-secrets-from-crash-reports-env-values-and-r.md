...
updated_at: 2026-08-20T05:03:46Z
parent: gosd-47z3
blocked_by:
    - gosd-pun9
---
...
- [x] Redact the WHOLE rendered body, not only the technical-detail section.
      ...
      known-safe fields and needs no pass~~ — **CORRECTED (2026-08-20, beans
      gosd-ywsv and gosd-fu1z).** Both halves of that sentence were wrong.
      `error_code` is `Report.Code`, arbitrary app-supplied text, so the
      frontmatter did need a pass; and redacting the whole rendered body
      swept gosd’s own compile-time prose, which cannot hold a secret and so
      could only be damaged (an env value of `computer` rewrote the sentence
      explaining what the file is). The rule now: redaction applies to every
      value the report carries in from outside gosd — `Report.Code`, `Doing`,
      `Problem`, `Fix`, `Detail`, and `Context.AppName`/`AppVersion`/
      `SupportURL` — applied per field before assembly, wherever each lands,
      frontmatter included; and to nothing gosd wrote or measured itself.

...
- **Shallow checkouts get specific guidance, and the recommended fix is a treeless fetch (amended 2026-08-24, JP)**: the no-matching-tag error detects a shallow clone and names `git fetch --unshallow --tags --filter=tree:0` (or actions/checkout `fetch-depth: 0` with `filter: tree:0`) — resolution needs the commit graph and tags but no historical file contents, and a bare `fetch-depth: 0` downloads every version of every file to answer a question about commits. Bench-verified against real git 2.54: the pure-Go resolver works on a treeless partial clone (missing tree/blob objects confirmed via `rev-list --missing=print`), and the repair command converts a depth-1 clone into a resolvable one. Servers without partial-clone support ignore the filter, so the treeless form never does worse. The same guidance lives in the build-config docs page. A plain no-match (not shallow) names the glob and how many tags exist.

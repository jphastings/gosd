...
- **Staged npm publishing pipeline** (added to the same PR on request): `.github/workflows/publish-npm.yml` — `npm/<package>/vX.Y.Z` tags → full verification at the tagged commit (incl. commit-on-main and tag-vs-manifest version checks, tarball listing) → human-approved `npm-publish` environment → OIDC trusted publishing (no npm token exists anywhere) with `--provenance`, only ever to the `next` dist-tag → registry smoke install verifying version + provenance signatures. Promotion to `latest` is a deliberate human 2FA action. Package-generic for future js/packages/*. Procedure + one-time setup + rollback: js/PUBLISHING.md; JP-side console steps tracked in gosd-xe3r.

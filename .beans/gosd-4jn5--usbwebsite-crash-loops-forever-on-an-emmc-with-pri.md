...
updated_at: 2026-07-24T07:50:46Z
---
...
- Added an exported emmc.ErrRefusedFormat sentinel (alongside the existing ErrNoEMMC) that the eMMC package's refusal path now wraps with %w, keeping its descriptive message text byte-for-byte identical (verified). usbwebsite detects the case with errors.Is(res.Err, emmc.ErrRefusedFormat), the same pattern the file already used for ErrNoEMMC one line above it - no string matching. emmc's own tests now pin both sides of the contract: TestRunRefusesForeignContentWithoutDestructive asserts errors.Is(err, ErrRefusedFormat), and TestRunFormatsBlankWithoutDestructive's existing err==nil check (now commented to say so explicitly) pins that a blank eMMC never wraps it.
- Without consent, a refusal result no longer exits: usbwebsite logs the emmc error plus one actionable line naming the exact gosd.toml [env] fix, then idles instead of exiting, since gosd-init restarts exited apps regardless of exit code. ErrNoEMMC is unaffected and still exits as before.
...
- Added examples/usbwebsite/main_test.go (the example had no prior tests) covering the consent-parsing helper (isAffirmative), per the "small test, no heavy harness" guidance; the sentinel-matching behavior itself is covered by emmc's own tests.

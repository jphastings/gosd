...
// asking the shim to shut down on its own terms, mirroring
// cloudflared.Kill exactly. Used only for a deliberate restart
// (Deps.RestartSignal firing, see supervise) — a network blip or crash that
// ends the shim on its own is observed entirely through Deps.Wait, never
// this. Like StartProcess, this needs no "linux" build tag:
// os.Process.Signal(syscall.SIGTERM) works identically on macOS, so it
// compiles and runs in tests there too.

...
// what lets Run's supervise loop rely on runEnv (TS_AUTHKEY only) being the
...
func TestStartProcessRunsAndIsolatesEnv(t *testing.T) {
	...
	if err := os.Setenv("GOSD_TSFUNNEL_TEST_PARENT_ONLY", "leaked-if-inherited"); err != nil {
		t.Fatalf("Setenv: %v", err)
	}
	defer func() { _ = os.Unsetenv("GOSD_TSFUNNEL_TEST_PARENT_ONLY") }()

	stdout := &syncBuffer{}
	stderr := &syncBuffer{}
	pid, err := StartProcess(
		sh,
		[]string{"-c", `echo "child says: $CHILD_ONLY_VAR"; echo "parent leak: $GOSD_TSFUNNEL_TEST_PARENT_ONLY"; echo to-stderr 1>&2`},
...
func TestStartProcessReturnsErrorForMissingBinary(t *testing.T) {
	if _, err := StartProcess("/no/such/binary/gosd-tsfunnel-test", nil, nil, &syncBuffer{}, &syncBuffer{}); err == nil {
...
// rather than run to completion. Mirrors cloudflared's own test of the same
// name.

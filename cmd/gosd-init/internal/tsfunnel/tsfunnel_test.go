...
// the shim is running terminates it via Kill, and the following restart
// uses the reset (base) backoff rather than whatever it had escalated to —
// a deliberate restart is not counted as a crash. Mirrors cloudflared's own
...
func TestSuperviseRestartSignalTerminatesRunningChildAndResetsBackoff(t *testing.T) {
	...
	if !waitForKillCount(procs, 1) {
		t.Fatal("restart signal never terminated the running shim")
	}
...
// while supervise is parked waiting for network-up must not cause the shim
// it eventually starts to be immediately killed. Mirrors cloudflared's own
// test of the same name.
func TestSuperviseRestartSignalWhileParkedCoalescesHarmlessly(t *testing.T) {
	...
	if !waitForPending(clock, 1) {
		t.Fatal("tailscale-funnel never parked waiting for network-up")
	}

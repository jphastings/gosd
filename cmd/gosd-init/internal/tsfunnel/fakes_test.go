...
// fakeClock mirrors cloudflared's own fakeClock (duplicated rather than
...
// fakeDirs records MkdirAll calls instead of touching a real filesystem,
// optionally scripted to fail (simulating a read-only /data, i.e. EROFS).
...
func newFakeDirs() *fakeDirs {
	return &fakeDirs{}
}

func (f *fakeDirs) MkdirAll(path string, _ os.FileMode) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.err != nil {
		return f.err
...
func (f *fakeDirs) dirsCreated() []string {
	f.mu.Lock()
	defer f.mu.Unlock()

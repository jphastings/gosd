...
type Deps struct {
	...
	// deliberate restart of the currently-supervised child (epic gosd-ojbm
	// decision 4 — the runtime-WiFi-join epic's ingress reconnect is the
	// first producer, wired by a sibling bean, not this package). Firing it
	// while a child is running terminates that child (via Kill) and resets
	// the restart backoff, since a deliberate restart is not a crash; firing
	// it while supervise is parked waiting for network-up, or during the
	// backoff sleep between children, coalesces harmlessly (see
	// restartsignal.Signal.Drain) since the next child start already picks
	// up whatever changed. Nil — the zero value, and every pre-existing
	// test's default — makes supervise behave exactly as it did before this
	// field existed.
...
func supervise(deps Deps, opts Options) {
	...
	for {
		...
		// during the previous backoff sleep below: the child about to start
...
// runOnce starts cloudflared once, waits for it to exit — or, if
...
func runOnce(deps Deps, opts Options, backoff *childbackoff.Backoff) {
	...
	status, err, restarted := restartsignal.WaitOrKill(deps.RestartSignal,
		...
		func() {
			deps.Log("cloudflared: restart requested; terminating pid %d", pid)
			if err := deps.Kill(pid); err != nil {
				deps.Log("cloudflared: terminating pid %d failed: %v", pid, err)
			}
		},
	)
	_ = stdout.Close()

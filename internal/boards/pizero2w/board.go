...
// EXT4Support implements boards.Board: unsupported. The stock pi-zero-2w
// kernel doesn't build CONFIG_EXT4_FS in (see COMPATIBILITY.md and
// internal/blockmount's remedyFor) - the data partition has no ext4
// driver to mount with.
func (board) EXT4Support() boards.EXT4Support {
	return boards.EXT4Support{
		Supported: false,
		Reason:    "the stock pi-zero-2w kernel doesn't build CONFIG_EXT4_FS in; keep the default FAT32 data partition, or build a custom kernel with ext4 support (see docs/custom-kernels.md)",
	}
}

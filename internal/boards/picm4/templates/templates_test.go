...
// Locked content, per bean gosd-1tk8 (plus the fleet-wide gosd-85pt and
// gosd-fnza I2C/SPI defaults): do not change these expectations without
...
const (
	...
	wantCmdlineTxt = "console=serial0,115200 quiet init=/init gosd.board=pi-cm4 panic=10"
)
...
func TestRenderConfigTxt_NeverAddsADwc2Overlay(t *testing.T) {
	got, err := RenderConfigTxt(ConfigTxtData{InitramfsName: "initramfs.cpio.zst"})
	if err != nil {
		t.Fatalf("RenderConfigTxt() error = %v", err)
	}
	if strings.Contains(got, "dtoverlay") {
		t.Errorf("RenderConfigTxt() = %q, want no dtoverlay line: UsbGadgetSupport is unsupported until this carrier is characterized", got)
	}
}

func TestRenderCmdlineTxt(t *testing.T) {
	got, err := RenderCmdlineTxt(CmdlineTxtData{Board: "pi-cm4", ConsoleBaud: 115200})
	if err != nil {
...
func TestRenderCmdlineTxt_ConsoleBaudOverride(t *testing.T) {
	got, err := RenderCmdlineTxt(CmdlineTxtData{Board: "pi-cm4", ConsoleBaud: 57600})
	if err != nil {
		t.Fatalf("RenderCmdlineTxt() error = %v", err)
	}
	want := "console=serial0,57600 quiet init=/init gosd.board=pi-cm4 panic=10"
	if got != want {

...
func TestArtifactsHasNoAutomaticFetchSource(t *testing.T) {
	refs := radxazero3e.New().Artifacts()
	...
	for _, want := range []string{"idbloader.img", "u-boot.itb", "Image", "rk3566-radxa-zero-3e.dtb"} {
		ref, ok := names[want]
		if !ok {
...
func TestBootFilesContents(t *testing.T) {
	...
	if !strings.Contains(string(extlinuxConf), "gosd.board=radxa-zero-3e") {
		t.Errorf("extlinux.conf = %q, want it to contain gosd.board=radxa-zero-3e", extlinuxConf)
	}
}
...
func TestFirmwareFilesIsEmpty(t *testing.T) {
	if got := radxazero3e.New().FirmwareFiles(boards.Artifacts{}); len(got) != 0 {

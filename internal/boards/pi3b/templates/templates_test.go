package templates

import (
	"strings"
	"testing"
)

// Locked content, per bean gosd-ypg1 (plus the fleet-wide gosd-85pt and
// gosd-fnza I2C/SPI defaults): do not change these expectations without
// updating that decision.
const (
	wantConfigTxt = "arm_64bit=1\n" +
		"kernel=kernel8.img\n" +
		"initramfs initramfs.cpio.zst followkernel\n" +
		"enable_uart=1\n" +
		"disable_splash=1\n" +
		"boot_delay=0\n" +
		"avoid_warnings=1\n" +
		"dtparam=i2c_arm=on\n" +
		"dtparam=spi=on\n"

	wantCmdlineTxt = "console=serial0,115200 quiet init=/init gosd.board=pi-3b panic=10"
)

func TestRenderConfigTxt(t *testing.T) {
	got, err := RenderConfigTxt(ConfigTxtData{InitramfsName: "initramfs.cpio.zst"})
	if err != nil {
		t.Fatalf("RenderConfigTxt() error = %v", err)
	}
	if got != wantConfigTxt {
		t.Errorf("RenderConfigTxt() = %q, want %q", got, wantConfigTxt)
	}
}

func TestRenderConfigTxt_NeverAddsADwc2Overlay(t *testing.T) {
	got, err := RenderConfigTxt(ConfigTxtData{InitramfsName: "initramfs.cpio.zst"})
	if err != nil {
		t.Fatalf("RenderConfigTxt() error = %v", err)
	}
	if strings.Contains(got, "dtoverlay") {
		t.Errorf("RenderConfigTxt() = %q, want no dtoverlay line: the 3B's USB port is hub-wired and can never be a peripheral", got)
	}
}

func TestRenderCmdlineTxt(t *testing.T) {
	got, err := RenderCmdlineTxt(CmdlineTxtData{Board: "pi-3b", ConsoleBaud: 115200})
	if err != nil {
		t.Fatalf("RenderCmdlineTxt() error = %v", err)
	}
	if got != wantCmdlineTxt {
		t.Errorf("RenderCmdlineTxt() = %q, want %q", got, wantCmdlineTxt)
	}
}

func TestRenderCmdlineTxt_IsSingleLine(t *testing.T) {
	got, err := RenderCmdlineTxt(CmdlineTxtData{Board: "pi-3b", ConsoleBaud: 115200})
	if err != nil {
		t.Fatalf("RenderCmdlineTxt() error = %v", err)
	}
	for i, r := range got {
		if r == '\n' {
			t.Fatalf("RenderCmdlineTxt() contains a newline at byte %d, want a single line: %q", i, got)
		}
	}
}

func TestRenderCmdlineTxt_ConsoleBaudOverride(t *testing.T) {
	got, err := RenderCmdlineTxt(CmdlineTxtData{Board: "pi-3b", ConsoleBaud: 57600})
	if err != nil {
		t.Fatalf("RenderCmdlineTxt() error = %v", err)
	}
	want := "console=serial0,57600 quiet init=/init gosd.board=pi-3b panic=10"
	if got != want {
		t.Errorf("RenderCmdlineTxt(ConsoleBaud: 57600) = %q, want %q", got, want)
	}
}

func TestRenderConfigTxt_InterpolatesInitramfsName(t *testing.T) {
	got, err := RenderConfigTxt(ConfigTxtData{InitramfsName: "custom-initramfs.cpio.zst"})
	if err != nil {
		t.Fatalf("RenderConfigTxt() error = %v", err)
	}
	want := "initramfs custom-initramfs.cpio.zst followkernel\n"
	if !strings.Contains(got, want) {
		t.Errorf("RenderConfigTxt() = %q, want it to contain %q", got, want)
	}
}

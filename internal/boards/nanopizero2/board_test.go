...
func TestName(t *testing.T) {
	if got := nanopizero2.New().Name(); got != "nanopi-zero2" {
		t.Errorf("Name() = %q, want nanopi-zero2", got)
	}
}

func TestArtifactsHasNoAutomaticFetchSource(t *testing.T) {
	refs := nanopizero2.New().Artifacts()
	...
	for _, want := range []string{"idbloader.img", "u-boot.itb", "Image", "rk3528-nanopi-zero2.dtb"} {
		ref, ok := names[want]
		if !ok {
...
func TestBootFilesContents(t *testing.T) {
	...
	if !strings.Contains(string(extlinuxConf), "gosd.board=nanopi-zero2") {
		t.Errorf("extlinux.conf = %q, want it to contain gosd.board=nanopi-zero2", extlinuxConf)
	}
	if !strings.Contains(string(extlinuxConf), "console=ttyS0,1500000n8") {
		t.Errorf("extlinux.conf = %q, want it to contain console=ttyS0,1500000n8", extlinuxConf)
	}
}
...
func TestBootFilesIgnoresUsbGadget(t *testing.T) {
	...
	if string(extlinuxWithout) != string(extlinuxWith) {
		t.Errorf("extlinux.conf differs between UsbGadget=false/true; this board has no USB controller in any numbered mainline kernel release yet, so there's no boot-time change to make")
	}
}
...
func TestBootFilesDefaultsConsoleBaudTo1500000(t *testing.T) {
	...
	if !strings.Contains(string(extlinuxConf), "console=ttyS0,1500000n8") {
		t.Errorf("extlinux.conf = %q, want it to default to console=ttyS0,1500000n8 when ConsoleBaud is unset", extlinuxConf)
	}
}
...
func TestBootFilesHonorsConsoleBaudOverride(t *testing.T) {
	...
	if !strings.Contains(string(extlinuxConf), "console=ttyS0,115200n8") {
		t.Errorf("extlinux.conf = %q, want it to contain console=ttyS0,115200n8 when ConsoleBaud=115200", extlinuxConf)
	}
...
func TestConsoleBaudSupportIsSupported(t *testing.T) {
	if got := nanopizero2.New().ConsoleBaudSupport(); !got.Supported {
...
func TestFirmwareFilesIsEmpty(t *testing.T) {
	if got := nanopizero2.New().FirmwareFiles(boards.Artifacts{}); len(got) != 0 {

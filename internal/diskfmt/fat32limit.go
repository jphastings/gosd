...
func checkFAT32Size(devicePath string, sizeBytes int64) error {
	...
	return fmt.Errorf("%s is %s, and GoSD cannot create a FAT32 volume larger than %s: its FAT32 formatter counts the sectors in each file allocation table in 16 bits, so a larger volume would be laid out with FATs far too small for it and silently corrupted; format this device as exFAT instead (disk.Options{Filesystem: disk.ExFAT}), which has no such limit and suits media this large",
		devicePath, gibibytes(sizeBytes), gibibytes(maxFAT32Bytes))
}
...
// gibibytes renders a byte count the way storage sizes are talked about, with
// enough precision to tell a device just over the FAT32 limit from one just
// under it.

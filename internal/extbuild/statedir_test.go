...
// container bind mounts mid-build - see kernelbuild's identical guard,
// gosd-l4y9) and must stay under the user's home so Docker Desktop / podman
// machine share it with their VMs.
func TestBuildRootFor(t *testing.T) {
	t.Run("darwin uses Application Support", func(t *testing.T) {
		...
		want := filepath.Join("Library", "Application Support", "gosd", "ext-build")
	...
	t.Run("linux honors XDG_STATE_HOME", func(t *testing.T) {
		...
		if root != filepath.Join("/home/u/.state", "gosd", "ext-build") {
	...
	t.Run("linux defaults to ~/.local/state", func(t *testing.T) {
		...
		want := filepath.Join(".local", "state", "gosd", "ext-build")
		if !strings.HasSuffix(root, want) {
